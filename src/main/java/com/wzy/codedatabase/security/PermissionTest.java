...
/**
 * 基于安全管理器SecurityManager的自定义权限控制
 * 此例中自定义了一个权限控制文字的输入，严格限制某些词语无法输入
 * his class demonstrates the custom WordCheckPermission.
...
public class PermissionTest
{
   ...
   {
      System.setProperty("java.security.policy", "com/wzy/codedatabase/security/PermissionTest.policy");//指定策略文件位置
...
class WordCheckTextArea extends JTextArea
{
   ...
   {
      WordCheckPermission p = new WordCheckPermission(text, "insert");//定义需要的权限
      SecurityManager manager = System.getSecurityManager();//获取安全管理器
      if (manager != null) manager.checkPermission(p);//检查权限是否满足
      super.append(text);
   }
}
